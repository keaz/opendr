...
use crate::fsm::{
    StateMachine, ReplicationProviderState, ReplicationProviderEvent, 
    ReplicationProviderFsm, ReplicationPhase
};
...
pub trait ChangelogProvider: Send + Sync {
    ...
    async fn get_all_entries(&self, base_dn: &str, filter: Option<&str>) -> Result<Vec<DirectoryEntry>, String>;
...
pub trait StreamingManager: Send + Sync {
    ...
    async fn start_streaming(&mut self, consumer_id: &str, start_cookie: Option<&str>) -> Result<(), String>;
...
pub trait ReplicationMetrics: Send + Sync {
    ...
    fn record_phase_complete(&self, consumer_id: &str, phase: &str, entries_processed: usize, duration: Duration);
    ...
    fn record_entry_streamed(&self, consumer_id: &str, entry_size: usize, processing_time: Duration);
    ...
    fn record_consumer_disconnection(&self, consumer_id: &str, reason: &str, session_duration: Duration);
...
pub trait SyncRequestHandler: Send + Sync {
    ...
    async fn generate_sync_response(&self, consumer_id: &str, result_code: u32, cookie: Option<&str>, entries_sent: usize) -> Result<SyncResponse, String>;
}
...
impl ReplicationProviderFsmImpl {
    ...
    async fn handle_start_sync_replication(
        ...
        if !matches!(self.state, ReplicationProviderState::Initializing) {
            return Err(ReplicationProviderError::InvalidStateTransition {
                ...
                to: ReplicationProviderState::Refresh { entries_sent: 0, total_entries: 0 },
            });
        }
        ...
        if self.sessions.len() >= self.config.max_concurrent_consumers as usize {
            return Err(ReplicationProviderError::Generic {
                message: format!("Maximum consumer limit ({}) reached", self.config.max_concurrent_consumers),
            });
        }
        ...
        // Register consumer
        self.consumer_registry.register_consumer(&consumer_id, connection.clone()).await
        ...
        self.sync_request_handler.validate_sync_request(&sync_request).await
        ...
        let entries = self.changelog_provider.get_all_entries(&base_dn, None).await
    ...
    async fn handle_refresh_complete(
        ...
        if !matches!(self.state, ReplicationProviderState::Refresh { .. }) {
            return Err(ReplicationProviderError::InvalidStateTransition {
                from: self.state.clone(),
                to: ReplicationProviderState::Present { entries_streamed: 0 },
            });
        }
        ...
        if let Some(session) = self.sessions.get_mut(&consumer_id) {
            ...
            // Record metrics
            if let Some(ref metrics) = self.metrics {
                metrics.record_phase_complete(&consumer_id, "refresh", entries_sent, session.session_duration());
            }
        }
        
        // Transition to present phase
        self.state = ReplicationProviderState::Present { entries_streamed: 0 };
    ...
    async fn handle_present_complete(
        ...
        if !matches!(self.state, ReplicationProviderState::Present { .. }) {
            return Err(ReplicationProviderError::InvalidStateTransition {
                from: self.state.clone(),
                to: ReplicationProviderState::Persist { cookie: String::new() },
            });
        }
        ...
        if let Some(session) = self.sessions.get_mut(&consumer_id) {
            ...
            // Record metrics
            if let Some(ref metrics) = self.metrics {
                metrics.record_phase_complete(&consumer_id, "present", entries_streamed, session.session_duration());
            }
        }
    ...
    async fn handle_changelog_entry(
        ...
        if !matches!(self.state, 
            ...
            return Err(ReplicationProviderError::InvalidStateTransition {
                from: self.state.clone(),
                to: ReplicationProviderState::Streaming { active_consumers: 1 },
            });
        }
        ...
        for consumer_id in &consumer_ids {
            match self.streaming_manager.send_entry(consumer_id, &changelog_entry).await {
                Ok(()) => {
                    ...
                    // Record metrics
                    if let Some(ref metrics) = self.metrics {
                        metrics.record_entry_streamed(consumer_id, entry_size, Duration::from_millis(1));
                    }
                },
                Err(e) => {
    ...
    async fn handle_consumer_disconnected(
        ...
        if let Some(session) = session {
            ...
            // Unregister consumer
            let _ = self.consumer_registry.unregister_consumer(&consumer_id).await;
    ...
    async fn handle_cookie_persisted(
        ...
        if matches!(self.state, ReplicationProviderState::Persist { .. }) {
            self.state = ReplicationProviderState::Persist { cookie: new_cookie.clone() };
        }
    ...
    async fn handle_error(&mut self, error_message: String) -> Result<Option<usize>, ReplicationProviderError> {
        // Update state to error
        self.state = ReplicationProviderState::Error { message: error_message.clone() };
        ...
        // Return error
        Err(ReplicationProviderError::Generic { message: error_message })
    }
}
...
impl StateMachine for ReplicationProviderFsmImpl {
    ...
    async fn handle_event(&mut self, event: Self::Event) -> Result<Option<Self::Output>, Self::Error> {
        match event {
            ReplicationProviderEvent::StartSyncReplication { consumer_id, cookie } => {
                self.handle_start_sync_replication(consumer_id, cookie).await
            },
            ...
            ReplicationProviderEvent::Error(message) => {
    ...
    async fn reset(&mut self) -> Result<(), Self::Error> {
        ...
        let active_consumers = self.consumer_registry.get_active_consumers().await
            .map_err(|e| ReplicationProviderError::RegistryError { message: e })?;
...
pub mod tests {
    ...
    impl ConsumerRegistry for MockConsumerRegistry {
        async fn register_consumer(&mut self, consumer_id: &str, connection_info: ConsumerConnection) -> Result<(), String> {
            if self.should_fail {
                ...
                self.consumers.insert(consumer_id.to_string(), connection_info);
                Ok(())
            }
        }
    ...
    impl SyncRequestHandler for MockSyncRequestHandler {
        ...
        async fn generate_sync_response(&self, _consumer_id: &str, result_code: u32, cookie: Option<&str>, entries_sent: usize) -> Result<SyncResponse, String> {
            if self.should_fail {
                Err("Mock sync handler failure".to_string())
            } else {
    ...
    impl ReplicationMetrics for MockReplicationMetrics {
        ...
        fn get_replication_stats(&self) -> ReplicationStats { ReplicationStats::new() }
    }
    ...
    async fn test_new_replication_provider_fsm() {
        ...
        assert!(matches!(fsm.current_state(), ReplicationProviderState::Initializing));
    ...
    async fn test_start_sync_replication_success() {
        ...
        assert!(matches!(fsm.current_state(), ReplicationProviderState::Refresh { entries_sent: 0, total_entries: 2 }));
    ...
    async fn test_start_sync_replication_registry_error() {
        ...
        assert!(matches!(result.unwrap_err(), ReplicationProviderError::RegistryError { .. }));
    }
    ...
    async fn test_refresh_complete_success() {
        ...
        assert!(matches!(fsm.current_state(), ReplicationProviderState::Present { entries_streamed: 0 }));
    ...
    async fn test_changelog_entry_streaming() {
        ...
        assert!(matches!(fsm.current_state(), ReplicationProviderState::Streaming { active_consumers: 1 }));
        assert!(fsm.is_streaming());
    ...
    async fn test_changelog_entry_no_consumers() {
        ...
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), ReplicationProviderError::NoActiveConsumer));
    }
    ...
    async fn test_entry_streamed_confirmation() {
        ...
        let result = fsm.handle_event(ReplicationProviderEvent::EntryStreamed {
            consumer_id: "consumer1".to_string(),
        }).await;
    ...
    async fn test_entry_streamed_consumer_not_found() {
        ...
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), ReplicationProviderError::ConsumerNotFound { .. }));
    }
    ...
    async fn test_cookie_persisted() {
        ...
        fsm.state = ReplicationProviderState::Persist { cookie: "old-cookie".to_string() };
        
        let result = fsm.handle_event(ReplicationProviderEvent::CookiePersisted {
    ...
    async fn test_error_event() {
        ...
        let result = fsm.handle_event(ReplicationProviderEvent::Error(error_message.to_string())).await;
        
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), ReplicationProviderError::Generic { .. }));
        assert!(matches!(fsm.current_state(), ReplicationProviderState::Error { .. }));
    ...
    async fn test_is_terminal_states() {
        ...
        // Set to error state
        fsm.state = ReplicationProviderState::Error { message: "test error".to_string() };
        assert!(fsm.is_terminal());
        
        // Non-terminal states
        fsm.state = ReplicationProviderState::Refresh { entries_sent: 0, total_entries: 10 };
        assert!(!fsm.is_terminal());
        
        fsm.state = ReplicationProviderState::Streaming { active_consumers: 1 };
        assert!(!fsm.is_terminal());
    }
    ...
    async fn test_directory_entry_methods() {
        ...
        assert_eq!(entry.uuid, Some("123e4567-e89b-12d3-a456-426614174000".to_string()));
    ...
    async fn test_sync_response_methods() {
        ...
        assert_eq!(response.message, Some("Sync completed successfully".to_string()));
    }
    ...
    async fn test_error_display() {
        let errors = vec![
            ...
            ReplicationProviderError::NoActiveConsumer,
            ReplicationProviderError::ConsumerNotFound { consumer_id: "test".to_string() },
            ReplicationProviderError::InvalidCookie { cookie: "invalid".to_string() },
            ReplicationProviderError::ChangelogError { message: "test".to_string() },
            ReplicationProviderError::RegistryError { message: "test".to_string() },
            ReplicationProviderError::StreamingError { message: "test".to_string() },
            ReplicationProviderError::SyncRequestError { message: "test".to_string() },
            ReplicationProviderError::Generic { message: "test".to_string() },
        ];
