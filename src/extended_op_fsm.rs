use crate::fsm::{ExtendedOpState, ExtendedOpEvent, ExtendedOpFsm, ExtendedOpResultCode, StateMachine};
...
#[derive(Debug, Clone)]
pub struct ExtendedOpError {
    message: String,
}

impl fmt::Display for ExtendedOpError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Extended operation error: {}", self.message)
    }
}
...
impl ExtendedOpFsmImpl {
    ...
    pub fn is_completed(&self) -> bool {
        matches!(self.state, ExtendedOpState::Completed { result_code: ExtendedOpResultCode::Success })
    }
    ...
    async fn handle_parsing(&mut self, oid: String, value: Option<Vec<u8>>) -> Result<(), ExtendedOpError> {
        // Record operation start
        self.start_time = Some(std::time::Instant::now());
        self.metrics.record_operation_start(&oid);
        
        // Check access control first
        if let Err(e) = self.access_control.check_permission(&oid, self.user_dn.as_deref()) {
            return Err(ExtendedOpError::from(format!("Access denied for operation {}: {}", oid, e)));
        }
        ...
        // Parse the operation
        let parsed = self.parser.parse_request(&oid, value.as_deref())
            .map_err(|e| ExtendedOpError::from(e))?;
        
        // Validate the parsed operation
        self.parser.validate_operation(&parsed)
            .map_err(|e| ExtendedOpError::from(e))?;
        
        // Check if backend supports this operation
        if !self.backend.is_operation_supported(&oid) {
            return Err(ExtendedOpError::from(format!("Operation {} not supported", oid)));
        }
    ...
    async fn handle_processing(&mut self, operation: String) -> Result<(), ExtendedOpError> {
        let _parsed_op = self.parsed_operation.as_ref()
            .ok_or_else(|| ExtendedOpError::from("No parsed operation available"))?;
        
        // Execute the operation through backend
        let response_data = self.backend.execute_operation(
            &operation,
            self.operation_value.as_deref()
        ).await.map_err(|e| ExtendedOpError::from(e))?;
    ...
    async fn handle_delegation(&mut self, operation: String, delegate: String) -> Result<(), ExtendedOpError> {
        let parsed_op = self.parsed_operation.as_ref()
            .ok_or_else(|| ExtendedOpError::from("No parsed operation available"))?;
        ...
        let response_data = self.delegator.delegate_operation(parsed_op).await
            .map_err(|e| ExtendedOpError::from(e))?;
    ...
    fn record_completion(&self, success: bool) {
        if let (Some(oid), Some(start_time)) = (&self.operation_oid, &self.start_time) {
            ...
            self.metrics.record_operation_complete(oid, success, duration);
        }
    }
}
...
impl StateMachine for ExtendedOpFsmImpl {
    ...
    async fn handle_event(&mut self, event: Self::Event) -> Result<Option<Self::Output>, Self::Error> {
        match (&self.state, &event) {
            ...
            (ExtendedOpState::Parsing, ExtendedOpEvent::StartExtendedOp { oid, value }) => {
                ...
                if let Some(parsed) = &self.parsed_operation {
                    if parsed.requires_delegation {
                        ...
                        self.state = ExtendedOpState::Processing { operation: oid.clone() };
                    }
                }
                Ok(None)
            }
            ...
            (ExtendedOpState::Delegating { operation, delegate }, ExtendedOpEvent::DelegationComplete) => {
                self.handle_delegation(operation.clone(), delegate.clone()).await?;
                self.state = ExtendedOpState::Responding;
                Ok(None)
            }
            ...
            _ => Err(ExtendedOpError::from(format!(
