...
use crate::fsm::{StateMachine, CompareFsm, CompareState, CompareEvent, CompareParams};
...
pub trait AttributeComparator: Send + Sync {
    ...
    fn is_case_insensitive(&self, attr_name: &str) -> bool {
        ...
        matches!(attr_name.to_lowercase().as_str(), 
            "cn" | "commonname" | "sn" | "surname" | "givenname" | 
            "displayname" | "description" | "o" | "organizationname" |
            "ou" | "organizationalunitname" | "l" | "localityname" |
            "st" | "stateorprovincename" | "street" | "postaladdress" |
            "postalcode" | "telephonenumber" | "mail" | "email"
        )
    }
}
...
impl CompareSession {
    ...
    pub fn entry_read_duration(&self) -> Option<Duration> {
        self.entry_read_time.map(|t| t.duration_since(self.start_time))
    }
...
impl CompareFsmImpl {
    ...
    fn validate_compare_params(&self, params: &CompareParams) -> Result<(), CompareFsmError> {
        ...
        if !self.config.allow_operational_attributes && self.is_operational_attribute(&params.attribute) {
            return Err(CompareFsmError::InvalidParameters {
                message: format!("Comparison of operational attribute '{}' is not allowed", params.attribute),
            });
        }
    ...
    fn is_operational_attribute(&self, attr_name: &str) -> bool {
        ...
        matches!(attr_name.to_lowercase().as_str(),
            ...
            "modifiersname" | "subschemasubentry" | "hassubordinates" |
    ...
    async fn handle_start_compare(
        ...
        let params = CompareParams { dn, attribute, value };
        ...
        if self.config.enable_access_control {
            self.access_control
                .check_compare_permission(None, &params.dn, &params.attribute)
                .await
                .map_err(|e| CompareFsmError::AccessDenied { message: e })?;
        }
        
        // Create new session
        let session = CompareSession::new(params.clone(), None);
    ...
    async fn handle_entry_read(&mut self) -> Result<Option<Vec<u8>>, CompareFsmError> {
        if let Some(session) = &mut self.session {
            ...
            // Retrieve entry with requested attribute only
            let entry_result = self.backend
                .get_entry_attributes(&session.params.dn, &[session.params.attribute.clone()])
                .await
                .map_err(|e| CompareFsmError::BackendError { message: e })?;
            ...
            match entry_result {
                ...
                None => {
                    ...
                    if self.config.enable_metrics {
                        if let Some(ref metrics) = self.metrics {
                            metrics.record_compare_error("NoSuchObject", session.total_duration());
                        }
                    }
                    
                    Err(CompareFsmError::NoSuchObject {
                        dn: session.params.dn.clone(),
                    })
                }
            }
        } else {
    ...
    async fn handle_error(&mut self, error_message: String) -> Result<Option<Vec<u8>>, CompareFsmError> {
        if let Some(session) = &self.session {
        ...
        self.state = CompareState::Completed { result: false };
        Err(CompareFsmError::Generic { message: error_message })
    }
}
...
impl StateMachine for CompareFsmImpl {
    ...
    async fn handle_event(&mut self, event: Self::Event) -> Result<Option<Self::Output>, Self::Error> {
        match event {
            CompareEvent::StartCompare { dn, attribute, value } => {
                self.handle_start_compare(dn, attribute, value).await
            },
            CompareEvent::EntryRead => {
                self.handle_entry_read().await
            },
            CompareEvent::ComparisonComplete(result) => {
                self.handle_comparison_complete(result).await
            },
            CompareEvent::ResultEmitted => {
                self.handle_result_emitted().await
            },
            CompareEvent::Error(error_message) => {
...
mod tests {
    ...
    impl MockCompareBackend {
        pub fn new() -> Self {
            ...
            entry1.add_attribute("objectClass".to_string(), vec![b"person".to_vec(), b"inetOrgPerson".to_vec()]);
    ...
    async fn test_start_compare_invalid_parameters() {
        ...
        assert!(matches!(result.unwrap_err(), CompareFsmError::InvalidParameters { .. }));
        assert_eq!(fsm.current_state(), &CompareState::Reading);
    ...
    async fn test_entry_read_success() {
        ...
        // Start compare first
        let _result = fsm.handle_event(CompareEvent::StartCompare {
            dn: "cn=john,dc=example,dc=org".to_string(),
            attribute: "mail".to_string(),
            value: b"john@example.org".to_vec(),
        }).await.unwrap();
        ...
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), None);
        assert_eq!(fsm.current_state(), &CompareState::Evaluating);
    }
    ...
    async fn test_entry_read_not_found() {
        ...
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), CompareFsmError::NoSuchObject { .. }));
        assert_eq!(fsm.current_state(), &CompareState::Completed { result: false });
    }

    #[tokio::test]
    async fn test_comparison_complete_true() {
        let backend = Box::new(MockCompareBackend::new());
        let comparator = Box::new(MockAttributeComparator::new());
        let access_control = Box::new(MockCompareAccessControl::new());
        ...
        let result = fsm.handle_event(CompareEvent::ComparisonComplete(true)).await;
    ...
    #[tokio::test]
    async fn test_comparison_complete_false() {
        let backend = Box::new(MockCompareBackend::new());
        let comparator = Box::new(MockAttributeComparator::new());
        let access_control = Box::new(MockCompareAccessControl::new());
        ...
        let _result = fsm.handle_event(CompareEvent::StartCompare {
            dn: "cn=john,dc=example,dc=org".to_string(),
            attribute: "mail".to_string(),
            value: b"wrong@example.org".to_vec(),
        }).await.unwrap();
        
        let _result = fsm.handle_event(CompareEvent::EntryRead).await.unwrap();
        
        // Handle comparison complete
        let result = fsm.handle_event(CompareEvent::ComparisonComplete(false)).await;
        
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), None);
        assert_eq!(fsm.current_state(), &CompareState::Emitting { result: false });
    ...
    async fn test_result_emitted() {
        ...
        let _result = fsm.handle_event(CompareEvent::EntryRead).await.unwrap();
        let _result = fsm.handle_event(CompareEvent::ComparisonComplete(true)).await.unwrap();
        ...
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), None);
        assert_eq!(fsm.current_state(), &CompareState::Completed { result: true });
        assert!(fsm.is_terminal());
        assert_eq!(fsm.result(), Some(true));
    ...
    async fn test_compare_error() {
        ...
        // Trigger error
        let result = fsm.handle_event(CompareEvent::Error("Test error".to_string())).await;
        
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), CompareFsmError::Generic { .. }));
        assert_eq!(fsm.current_state(), &CompareState::Completed { result: false });
        assert!(fsm.is_terminal());
    }
    ...
    async fn test_compare_with_metrics() {
        ...
        assert!(calls.iter().any(|call| call.contains("record_compare_start")));
    }
    ...
    async fn test_access_control_denial() {
        ...
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), CompareFsmError::AccessDenied { .. }));
    }
    ...
    async fn test_compare_session_methods() {
        ...
        let mut session = CompareSession::new(params, Some("cn=admin,dc=example,dc=org".to_string()));
