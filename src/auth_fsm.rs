...
use crate::fsm::{StateMachine, AuthState, AuthEvent, AuthFsm, AuthLevel};
...
impl AuthFsmImpl {
    ...
    async fn handle_bind_request(&mut self, dn: String, password: Vec<u8>) -> Result<Option<AuthUserInfo>, AuthError> {
    ...
    async fn handle_auth_success(&mut self) -> Result<Option<AuthUserInfo>, AuthError> {
        let dn = match &self.state {
            ...
            _ => {
                return Err(AuthError::InvalidStateTransition {
                    from: self.state.clone(),
                    to: AuthState::SimpleBound { dn: "unknown".to_string() }
                });
            }
        };
...
impl StateMachine for AuthFsmImpl {
    ...
    async fn handle_event(&mut self, event: Self::Event) -> Result<Option<Self::Output>, Self::Error> {
        match event {
            AuthEvent::BindRequest { dn, password } => {
                self.handle_bind_request(dn, password).await
            }
            AuthEvent::AuthenticationSuccess => {
                self.handle_auth_success().await
            }
            AuthEvent::AuthenticationFailure => {
                self.handle_auth_failure().await
            }
            AuthEvent::Unbind => {
                self.handle_unbind().await
            }
            AuthEvent::Reset => {
                self.reset().await?;
                Ok(None)
...
mod tests {
    ...
    impl MockAuthBackend {
        fn new() -> Self {
            ...
            valid_users.insert("cn=user1,dc=example,dc=org".to_string(), b"password123".to_vec());
    ...
    async fn test_invalid_state_transitions() {
        ...
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), AuthError::InvalidStateTransition { .. }));
    }
    ...
    async fn test_statistics_tracking() {
        ...
        assert_eq!(fsm.stats().anonymous_binds, initial_stats.anonymous_binds + 1);
        ...
        let _ = fsm.handle_event(AuthEvent::AuthenticationSuccess).await;
        assert_eq!(fsm.stats().successful_auths, initial_stats.successful_auths + 1);
        ...
        assert_eq!(fsm.stats().unbind_operations, initial_stats.unbind_operations + 1);
    }
    ...
    async fn test_mock_backend_validation() {
        ...
        // Test authentication
        let result = backend.authenticate("cn=admin,dc=example,dc=org", b"secret").await;
        assert!(result.is_ok());
        assert!(result.unwrap());
        
        let result = backend.authenticate("cn=admin,dc=example,dc=org", b"wrong").await;
        assert!(result.is_ok());
        assert!(!result.unwrap());
