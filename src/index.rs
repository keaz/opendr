...
type Key = i32; // For simplicity, we use i32 as the key type.
...
impl BTreeNode {
    fn search(&self, key: Key) -> Option<&Key> {
        let mut i = 0;
...
impl BTree {
    pub fn search(&self, key: Key) -> Option<&Key> {
        match &self.root {
...
impl BTreeNode {
    ...
    fn insert_non_full(&mut self, key: Key, t: usize) {
