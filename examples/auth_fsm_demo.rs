...
use opendr::auth_fsm::{
    AuthFsmImpl, AuthConfig, AuthenticationBackend, AuthUserInfo, AuthError
};
use opendr::fsm::{StateMachine, AuthEvent, AuthState, AuthFsm};
...
impl ExampleAuthBackend {
    ...
    pub fn new() -> Self {
        ...
        // Add sample users
        users.insert("cn=admin,dc=example,dc=org".to_string(), UserRecord {
        ...
        users.insert("cn=alice,ou=people,dc=example,dc=org".to_string(), UserRecord {
        ...
        users.insert("cn=bob,ou=people,dc=example,dc=org".to_string(), UserRecord {
...
impl AuthenticationBackend for ExampleAuthBackend {
    async fn authenticate(&self, dn: &str, password: &[u8]) -> Result<bool, String> {
        ...
        if let Some(user_record) = self.users.get(dn) {
            ...
            println!("✅ Authentication {} for {}", 
                if is_valid { "SUCCESS" } else { "FAILED" }, dn);
            Ok(is_valid)
        } else {
...
async fn demo_successful_authentication() -> Result<(), AuthError> {
    ...
    println!("📊 State after bind request: {:?}", auth_fsm.current_state());
    assert!(matches!(auth_fsm.current_state(), AuthState::Authenticating { .. }));
    ...
    let result = auth_fsm.handle_event(AuthEvent::AuthenticationSuccess).await?;
...
async fn demo_failed_authentication() -> Result<(), AuthError> {
    ...
    let result = auth_fsm.handle_event(AuthEvent::AuthenticationFailure).await?;
...
async fn demo_rate_limiting() -> Result<(), AuthError> {
    ...
    let _ = auth_fsm.handle_event(AuthEvent::AuthenticationFailure).await?;
    ...
    match auth_fsm.handle_event(AuthEvent::BindRequest {
        dn: "cn=alice,ou=people,dc=example,dc=org".to_string(),
...
async fn demo_full_lifecycle() -> Result<(), AuthError> {
    ...
    println!("   Session duration: {:?}", stats.session_start_time.elapsed());
...
mod tests {
    ...
    async fn test_example_backend() {
        ...
        let result = backend.authenticate("cn=alice,ou=people,dc=example,dc=org", b"alice_password").await;
        assert!(result.unwrap());
        
        // Test invalid password
        let result = backend.authenticate("cn=alice,ou=people,dc=example,dc=org", b"wrong").await;
        assert!(!result.unwrap());
        
        // Test nonexistent user
        let result = backend.authenticate("cn=nobody,dc=example,dc=org", b"password").await;
        assert!(!result.unwrap());
