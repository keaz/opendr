...
use opendr::fsm::{StateMachine, BerDecoderEvent, BerDecoderFsm};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let message1 = vec![0x04, 0x0C, b'H', b'e', b'l', b'l', b'o', b' ', b'W', b'o', b'r', b'l', b'd', b'!']; // "Hello World!"
    
    match fsm.handle_event(BerDecoderEvent::DataReceived(message1)).await {
        Ok(Some(decoded)) => {
    ...
    match fsm.handle_event(BerDecoderEvent::DataReceived(message2)).await {
        Ok(Some(decoded)) => {
    ...
    match fsm.handle_event(BerDecoderEvent::DataReceived(message3)).await {
        Ok(Some(decoded)) => {
    ...
    println!("   State: {:?}, Bytes needed: {:?}", fsm.current_state(), fsm.bytes_needed());
    
    println!("   Step 2: Sending length (0x08)...");
    let result2 = fsm.handle_event(BerDecoderEvent::DataReceived(vec![0x08])).await;
    println!("   State: {:?}, Bytes needed: {:?}", fsm.current_state(), fsm.bytes_needed());
    
    println!("   Step 3: Sending partial value...");
    let result3 = fsm.handle_event(BerDecoderEvent::DataReceived(vec![b'P', b'a', b'r', b't'])).await;
    println!("   State: {:?}, Bytes needed: {:?}", fsm.current_state(), fsm.bytes_needed());
    
    println!("   Step 4: Sending remaining value...");
    let result4 = fsm.handle_event(BerDecoderEvent::DataReceived(vec![b'i', b'a', b'l', b'!'])).await;
    ...
    match fsm.handle_event(BerDecoderEvent::DataReceived(long_message)).await {
        Ok(Some(decoded)) => {
            ...
            println!("   Tag: 0x{:02X}", decoded[0]);
            println!("   Length encoding: [0x{:02X}, 0x{:02X}, 0x{:02X}]", decoded[1], decoded[2], decoded[3]);
    ...
    println!("   Progress: tag={:?}, length={:?}", progress.tag, progress.length);
    ...
    match custom_fsm.handle_event(BerDecoderEvent::DataReceived(oversized)).await {
