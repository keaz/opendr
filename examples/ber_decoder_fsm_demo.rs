...
use opendr::ber_decoder_fsm::{
    BerDecoderConfig, BerDecoderFsmImpl, BerMessageHandler, BerValidator,
};
use opendr::fsm::{BerDecoderEvent, BerDecoderFsm, BerDecodingProgress, StateMachine};
...
impl BerMessageHandler for LdapMessageHandler {
    ...
    async fn on_progress_update(&mut self, progress: &BerDecodingProgress) -> Result<(), String> {
        if let (Some(tag), Some(length)) = (progress.tag, progress.length) {
            ...
            println!(
                "📊 Progress: {:.1}% ({}/{} bytes) for tag 0x{:02X}",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match fsm
        .handle_event(BerDecoderEvent::DataReceived(octet_string))
        .await
    {
        Ok(Some(message)) => {
    ...
    match fsm
        .handle_event(BerDecoderEvent::DataReceived(sequence))
        .await
    {
        Ok(Some(message)) => {
    ...
    let result2 = fsm
        .handle_event(BerDecoderEvent::DataReceived(vec![0x06]))
        .await;
    println!("   Result: {:?}", result2.is_ok());

    println!("   Sending partial value...");
    let result3 = fsm
        .handle_event(BerDecoderEvent::DataReceived(vec![b'W', b'o', b'r']))
        .await;
    println!("   Result: {:?}", result3.is_ok());

    println!("   Sending remaining value...");
    let result4 = fsm
        .handle_event(BerDecoderEvent::DataReceived(vec![b'l', b'd', b'!']))
        .await;
    match result4 {
        Ok(Some(message)) => {
    ...
    match fsm
        .handle_event(BerDecoderEvent::DataReceived(context_tag))
        .await
    {
        Ok(Some(message)) => {
    ...
    match fsm
        .handle_event(BerDecoderEvent::DataReceived(empty_message))
        .await
    {
        Ok(Some(message)) => {
    ...
    println!(
        "   Progress: tag={:?}, length={:?}, received={}, needed={:?}",
    ...
    match fsm
        .handle_event(BerDecoderEvent::DataReceived(invalid_data))
        .await
    {
