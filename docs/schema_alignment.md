...
- RFC 4523 X.509 certificate, CRL, certificate pair, supported algorithm, PKI
  ...
  values in GeneralSubtree bases, certificate-pair component assertions, CRL
  issuer, CRL date, CRL-number ranges, CRL authority key identifier, reason
  flags, full-name distribution points, and name-relative-to-CRL-issuer
  distribution points. Remaining RFC 4523 unsupported open-type `otherName`
  values and X.400/EDI general-name components remain deferred.
...
- GitHub issue #198: partially completed RFC 4523 X.509 certificate schema
  ...
  unsupported open-type `otherName` values and X.400/EDI general-name component
  matching.
