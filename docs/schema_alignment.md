...
- RFC 4523 X.509 certificate, CRL, certificate pair, supported algorithm, PKI
  ...
  `ADMD`, `PRMD`, `X121`, `T-ID`, `O`, `OU`, `UA-ID`, `S`, `G`, `I`, `GQ`) in
  GeneralSubtree bases, certificate-pair component assertions, CRL issuer, CRL
  date, CRL-number ranges, CRL authority key identifier, reason flags,
  full-name distribution points, and name-relative-to-CRL-issuer distribution
  points. Remaining RFC 4523 constructed or schema-specific open-type
  `otherName` values plus X.400 domain-defined and extension attributes remain
  deferred.
...
- GitHub issue #198: partially completed RFC 4523 X.509 certificate schema
  ...
  domain-defined and extension attribute component matching.
