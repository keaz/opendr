...
- RFC 4523 X.509 certificate, CRL, certificate pair, supported algorithm, PKI
  ...
  key usage, subject alternative name type, certificate policy, name constraints
  including asserted GeneralSubtree minimum/maximum bounds, certificate-pair
  component assertions, CRL issuer, CRL date, CRL-number ranges, CRL authority
  key identifier, reason flags, and full-name distribution points. Remaining RFC
  4523 path-to-name, `otherName` value, X.400/EDI general-name, and
  name-relative-to-CRL-issuer components remain deferred.
...
- GitHub issue #198: partially completed RFC 4523 X.509 certificate schema
  ...
  uncommon X.509 path-to-name, `otherName` value, X.400/EDI general-name, and
  name-relative-to-CRL-issuer components.
