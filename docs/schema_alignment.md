...
- RFC 4523 X.509 certificate, CRL, certificate pair, supported algorithm, PKI
  ...
  key identifiers, validity, subject-public-key algorithm, key usage, subject
  alternative name type, certificate policy, certificate-pair component
  assertions, CRL issuer, CRL date, CRL-number ranges, CRL authority key
  identifier, reason flags, and full-name distribution points. Remaining RFC
  4523 private-key-validity, path-to-name, name-constraint, `otherName` value,
...
- GitHub issue #198: partially completed RFC 4523 X.509 certificate schema
  ...
  uncommon X.509 private-key-validity, path-to-name, name-constraint,
  `otherName` value, X.400/EDI general-name, and
