...
- RFC 4523 X.509 certificate, CRL, certificate pair, supported algorithm, PKI
  ...
  base-subtree name constraints, certificate-pair component assertions, CRL
  issuer, CRL date, CRL-number ranges, CRL authority key identifier, reason
  flags, and full-name distribution points. Remaining RFC 4523 path-to-name,
  GeneralSubtree min/max, `otherName` value, X.400/EDI general-name, and
...
- GitHub issue #198: partially completed RFC 4523 X.509 certificate schema
  ...
  uncommon X.509 path-to-name, GeneralSubtree min/max, `otherName` value,
  X.400/EDI general-name, and name-relative-to-CRL-issuer components.
