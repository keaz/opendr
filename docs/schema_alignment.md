...
- RFC 4523 X.509 certificate, CRL, certificate pair, supported algorithm, PKI
  ...
  GeneralSubtree minimum/maximum bounds and `otherName` string/OCTET STRING
  values plus `ediPartyName` values in GeneralSubtree bases, certificate-pair
  ...
  unsupported open-type `otherName` values and X.400 general-name components
  remain deferred.
...
- GitHub issue #198: partially completed RFC 4523 X.509 certificate schema
  ...
  unsupported open-type `otherName` values and X.400 general-name component
  matching.
