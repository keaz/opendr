...
| RFC 4527 | Post-Read control | `1.3.6.1.1.13.2` | `supportedControl` | Supported | Advertised. Add, modify, and ModifyDN may request a BER `AttributeSelection`; successful responses include a same-OID response control containing a BER `SearchResultEntry` for the post-change target entry. Critical requests on other operations are rejected with `unavailableCriticalExtension`; non-critical requests on other operations are ignored. | `read_entry_controls::tests::*`, `server::tests::add_with_post_read_returns_added_entry_control`, `server::tests::modify_with_pre_and_post_read_returns_prior_and_current_entry_controls`, `server::tests::moddn_with_post_read_returns_renamed_entry_control`, `fsm_server::tests::handle_connection_processes_*_with_post_read_control`, `tests/rfc_controls_extensions_integration.rs` |
...
### Pre-Read And Post-Read Controls
...
OpenDR also supports RFC 4527 Post-Read request controls on add, modify, and
ModifyDN operations. The request control value is decoded as a BER
`AttributeSelection`. On a successful update, OpenDR emits a response control
with OID `1.3.6.1.1.13.2` whose value is a BER `SearchResultEntry` containing
the target entry as it exists after the update.

Rules:

- Root DSE advertises `1.3.6.1.1.13.1` and `1.3.6.1.1.13.2` in
  `supportedControl`.
...
- Malformed Pre-Read or Post-Read request values return LDAP `protocolError` before the
  entry is mutated.
...
- Critical Post-Read requests on unsupported operations return
  `unavailableCriticalExtension`; non-critical Post-Read requests on those
  operations are ignored.

## Unsupported Controls

The assertion control is intentionally not registered as a supported request
control. This keeps Root DSE truthful and relies on RFC 4511 generic control
semantics:

- Unknown non-critical request controls are ignored.
- Unknown critical request controls return `unavailableCriticalExtension`.

OpenDR does not evaluate assertion-control preconditions.
