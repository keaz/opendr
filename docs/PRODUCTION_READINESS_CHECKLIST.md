...
| OpenLDAP CLI interoperability and RFC 4513 gate | `scripts/ldap_interop_gate.sh` | `ldapsearch`, `ldapadd`, `ldapmodify`, `ldapdelete`, `ldapcompare`, `ldapmodrdn`, and `ldapwhoami` cover Bind, StartTLS, Root DSE, Search, Add, Modify, Delete, ModifyDN, Compare, paged results, server-side sort, schema, operational attributes, cleartext-bind rejection, SASL PLAIN over StartTLS/LDAPS, granted and denied proxy authzid policy, and WhoAmI after simple and SASL bind. | Sanitized script transcript and server logs |
...
| SASL EXTERNAL mTLS interoperability | `scripts/ldap_interop_gate.sh` | Generated client-certificate fixtures validate `supportedSASLMechanisms: EXTERNAL`, SASL EXTERNAL bind, explicit self-authzid, granted and denied proxy-authzid policy, and WhoAmI over StartTLS and LDAPS. OpenLDAP mTLS is required when `OPENDR_INTEROP_REQUIRE_OPENLDAP_MTLS=1`; otherwise SecureTransport clients may use the raw Python TLS fallback. | Sanitized script transcript and server logs |
...
| Production config hardening | `scripts/production_config_gate.sh /etc/opendr/provider/server.toml` and equivalent consumer configs | Candidate production configs use `security.profile = "production"`, TLS, audit, access control deny-by-default rules, rate limiting, non-inline secret sources, absolute runtime paths outside the source tree, and no cleartext or anonymous bind override. | Gate log and final redacted config review |
...
| --- | --- | --- | --- | --- |
| Release performance regression-100k | `PERF_GATE_MODE=release PERF_GATE_BASELINE_JSON=target/perf/regression-baseline/opendr/regression-100k/ldap-benchmark-results.json PERF_GATE_OUTPUT_DIR=target/perf/regression-candidate ./scripts/perf_regression_gate.sh` | passed | `target/perf/regression-candidate/20260416T073953Z/comparison-summary.md` | Release-mode candidate pass on 2026-04-16: 68.24s runtime, 100,000 preloaded users, 100,005 records after setup, 252.38 MiB DB, and 268.50 MiB peak RSS under the 4 GiB limit. This was candidate-only because the retained baseline JSON was missing. |
| 1-hour replication soak | `SOAK_DURATION_SECS=3600 SOAK_ARTIFACT_DIR=target/replication-soak/1h-20260416T080507Z ./e2e_tests/test_replication_soak.sh` | passed | `target/replication-soak/1h-20260416T080507Z/summary.txt` | Passed for 3600 seconds across 294 rounds: 1470 adds, 588 modifies, 98 deletes, and final provider/consumer counts both 1372. Provider/consumer server logs, audit logs, and generated configs were retained in the artifact directory. |
| Full release fuzz budget | `FUZZ_GATE_MODE=release FUZZ_GATE_OUTPUT_DIR=target/fuzz-gate/release-20260416T090940Z ./scripts/fuzz_gate.sh` | passed | `target/fuzz-gate/release-20260416T090940Z/summary.md` | Passed on 2026-04-16 with `ber_decoder` and `ldap_request_handler` each running 21601 seconds. `ber_decoder` executed 289,376,720 units with 632 MiB peak RSS; `ldap_request_handler` executed 26,493,560 units with 618 MiB peak RSS. No crash artifacts were produced. |
