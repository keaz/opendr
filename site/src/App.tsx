...
opendr-setup --config-dir ./config generate-schema --bundle all --overwrite`}</code></pre>
                  <KeyValueTable
                    ...
                    rows={[
                      ...
                      ["x509", "RFC 4523 X.509 certificate, CRL, certificate pair, supported algorithm, PKI user, PKI CA, and CRL distribution point schema. DER-backed values are validated; exact and component GSER assertions cover certificate identity, key identifiers, validity, private-key validity, key usage, subject alternative name types, policies, path-to-name checks, otherName BOOLEAN, INTEGER, BIT STRING, NULL, object identifier, string, and OCTET STRING values, ediPartyName values, common X.400 ORAddress built-in attributes, name constraints with asserted GeneralSubtree bounds, CRL date/range, CRL authority keys, full-name and name-relative distribution points, certificate pairs, and algorithm OIDs."],
                    ]}
                  />
                </section>
              </div>
