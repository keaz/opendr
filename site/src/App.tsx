...
const operationsRows = [
  ...
  ["Controls and features", "Paged results, server-side sort, ManageDsaIT, Subentries, LDAP Sync controls, and RFC 4529 object-class attribute selection are supported."],
...
opendr-setup hash-password 'StrongPass123'`}</code></pre>
              ...
              <ul>
                ...
                <li>Bundled schema LDIF files such as <code>core/rfc4517.ldif</code>, <code>core/rfc4519.ldif</code>, <code>core/rfc2798.ldif</code>, <code>core/rfc3672.ldif</code>, <code>core/rfc3671.ldif</code>, <code>posix/rfc2307.ldif</code>, <code>cosine/rfc4524.ldif</code>, and <code>x509/rfc4523.ldif</code> when <code>generate-schema</code> is used.</li>
...
opendr --config config/server.toml schema explain exampleEmployeeNumber`}</code></pre>
                ...
                <section className="config-section">
                  <h3><code>Bundled schema</code></h3>
                  <p>
                    Standard schema bundles can be loaded directly or generated
                    into <code>config/schema</code> for packaging and review.
                  </p>
                  <pre><code>{`[schema]
load_builtin = ["core", "posix", "cosine", "x509"]

opendr-setup --config-dir ./config generate-schema --bundle all --overwrite`}</code></pre>
                  <KeyValueTable
                    headings={["Bundle", "Standard coverage"]}
                    rows={[
                      ["core", "File-backed RFC 4517 syntaxes and matching rules, RFC 4519 core user schema, RFC 2798 inetOrgPerson, RFC 3672 LDAP subentries, and RFC 3671 collective attributes."],
                      ...
                      ["x509", "RFC 4523 X.509 certificate, CRL, certificate pair, supported algorithm, PKI user, PKI CA, and CRL distribution point schema. DER-backed values are validated; exact and component GSER assertions cover certificate identity, key identifiers, validity, private-key validity, key usage, subject alternative name types, policies, path-to-name checks, otherName BOOLEAN, INTEGER, BIT STRING, NULL, object identifier, string, OCTET STRING, RFC 4043 permanentIdentifier, and RFC 4108 hardwareModuleName values, ediPartyName values, X.400 ORAddress built-in standard, domain-defined, and RFC 2156-renderable extension attributes including NET-PSAP, name constraints with asserted GeneralSubtree bounds, CRL date/range, CRL authority keys, full-name and name-relative distribution points, certificate pairs, and algorithm OIDs."],
                    ]}
                  />
                </section>
              </div>
