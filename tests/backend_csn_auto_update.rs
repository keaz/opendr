...
use opendr::backend::{DirectoryBackend, DirectoryEntry, Modification, ModifyOperation, MockBackend};
...
async fn test_add_entry_generates_csn_mock() {
    ...
    let stored = backend.get_entry("cn=jdoe,dc=example,dc=org").await.unwrap().unwrap();
    assert!(stored.operational_attributes.entry_csn.is_some(), "entryCSN should be set");
    assert!(stored.operational_attributes.create_timestamp.is_some(), "createTimestamp should be set");
    assert!(stored.operational_attributes.modify_timestamp.is_some(), "modifyTimestamp should be set");
...
async fn test_modify_entry_updates_csn_mock() {
    ...
    let original_entry = backend.get_entry("cn=jdoe,dc=example,dc=org").await.unwrap().unwrap();
    let original_csn = original_entry.operational_attributes.entry_csn.clone();
    let original_modify_time = original_entry.operational_attributes.modify_timestamp.clone();
    ...
    backend.modify_entry("cn=jdoe,dc=example,dc=org", modifications).await.unwrap();
    
    // Verify entryCSN was updated
    let modified_entry = backend.get_entry("cn=jdoe,dc=example,dc=org").await.unwrap().unwrap();
    let modified_csn = modified_entry.operational_attributes.entry_csn.clone();
    let modified_modify_time = modified_entry.operational_attributes.modify_timestamp.clone();
    
    assert!(modified_csn.is_some(), "entryCSN should still be set");
    assert_ne!(original_csn, modified_csn, "entryCSN should have changed");
    assert_ne!(original_modify_time, modified_modify_time, "modifyTimestamp should have changed");
    
    // Verify contextCSN was updated
    let context_csn = backend.get_context_csn().await.unwrap();
    assert!(context_csn.is_some(), "contextCSN should be updated after modify");
}
...
async fn test_delete_entry_updates_context_csn_mock() {
    ...
    // Delete entry
    backend.delete_entry("cn=jdoe,dc=example,dc=org").await.unwrap();
    
    // Verify contextCSN was updated
    let context_csn_after = backend.get_context_csn().await.unwrap();
    assert!(context_csn_after.is_some(), "contextCSN should be set after delete");
    assert_ne!(context_csn_before, context_csn_after, "contextCSN should have changed after delete");
}
...
async fn test_rename_entry_updates_csn_mock() {
    ...
    // Rename entry
    backend.rename_entry("cn=jdoe,dc=example,dc=org", "cn=johndoe", false, None).await.unwrap();
    
    // Verify entryCSN was updated
    let renamed_entry = backend.get_entry("cn=johndoe,dc=example,dc=org").await.unwrap().unwrap();
    let renamed_csn = renamed_entry.operational_attributes.entry_csn.clone();
    
    assert!(renamed_csn.is_some(), "entryCSN should still be set after rename");
    assert_ne!(original_csn, renamed_csn, "entryCSN should have changed after rename");
    
    // Verify contextCSN was updated
    let context_csn = backend.get_context_csn().await.unwrap();
    assert!(context_csn.is_some(), "contextCSN should be updated after rename");
}
...
async fn test_csn_ordering() {
    ...
    for i in 0..5 {
        let entry = backend.get_entry(&format!("cn=user{},dc=example,dc=org", i))
            .await.unwrap().unwrap();
...
async fn test_context_csn_reflects_latest_change() {
    ...
    assert!(csn2 > csn1, "contextCSN should increase with each write operation");
}
