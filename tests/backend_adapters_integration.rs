...
use opendr::backend_adapters::{SearchBackendAdapter, WriteBackendAdapter, CompareBackendAdapter};
use opendr::search_fsm::SearchBackend;
use opendr::write_fsm::{WriteBackend, Modification};
...
async fn test_search_backend_adapter() {
    ...
    let candidates = adapter.find_candidates("dc=example,dc=org", 2, "(cn=testuser)").await.unwrap();
    assert!(candidates.len() > 0, "Should find at least one candidate");

    // Test get_entry
    let entry = adapter.get_entry("cn=testuser,dc=example,dc=org", &["cn".to_string()]).await.unwrap();
    ...
    // Test entry_exists
    let exists = adapter.entry_exists("cn=testuser,dc=example,dc=org").await.unwrap();
    assert!(exists, "Entry should exist");

    let not_exists = adapter.entry_exists("cn=nonexistent,dc=example,dc=org").await.unwrap();
...
async fn test_write_backend_adapter() {
    ...
    let txn_id = adapter.begin_transaction().await.unwrap();
    adapter.rollback_transaction(&txn_id, "test rollback").await.unwrap();

    // Test validate_entry
    let entry_data = b"dn: cn=newuser,dc=example,dc=org\ncn: newuser\nobjectClass: person\n";
    adapter.validate_entry("cn=newuser,dc=example,dc=org", entry_data).await.unwrap();

    // Test add_entry
    let txn_id = adapter.begin_transaction().await.unwrap();
    adapter.add_entry(&txn_id, "cn=newuser,dc=example,dc=org", entry_data).await.unwrap();

    // Test entry_exists
    let exists = adapter.entry_exists("cn=newuser,dc=example,dc=org").await.unwrap();
    ...
    adapter.modify_entry(&txn_id, "cn=newuser,dc=example,dc=org", &modifications).await.unwrap();

    // Test delete_entry
    let txn_id = adapter.begin_transaction().await.unwrap();
    adapter.delete_entry(&txn_id, "cn=newuser,dc=example,dc=org").await.unwrap();
...
async fn test_compare_backend_adapter() {
    ...
    let entry = adapter.get_entry_attributes(
    ...
    let stats = adapter.get_compare_stats("cn=testuser,dc=example,dc=org").await.unwrap();
    assert_eq!(stats, (0, 0), "Stats should return (0, 0) for now");
}
...
async fn test_write_backend_adapter_modify_dn() {
    ...
    let txn_id = adapter.begin_transaction().await.unwrap();
    adapter.add_entry(&txn_id, "cn=oldname,dc=example,dc=org", entry_data).await.unwrap();
    ...
    let old_exists = adapter.entry_exists("cn=oldname,dc=example,dc=org").await.unwrap();
    assert!(!old_exists, "Old entry should not exist after rename");

    // Verify new entry exists
    let new_exists = adapter.entry_exists("cn=newname,dc=example,dc=org").await.unwrap();
