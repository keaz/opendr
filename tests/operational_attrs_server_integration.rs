...
async fn test_e2e_add_and_search_with_operational_attrs() {
    ...
    let result = adapter.get_entry("cn=Test User,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    
    // Verify operational attributes are present
    assert!(result.attributes.contains_key("entrycsn"), "Should have entryCSN");
    assert!(result.attributes.contains_key("createtimestamp"), "Should have createTimestamp");
    assert!(result.attributes.contains_key("modifytimestamp"), "Should have modifyTimestamp");
    
    // Verify user attributes are NOT present when only "+" is requested
    assert!(!result.attributes.contains_key("cn"), "Should not have user attrs with only '+'");
    assert!(!result.attributes.contains_key("mail"), "Should not have user attrs with only '+'");
}
...
async fn test_e2e_modify_updates_operational_attrs() {
    ...
    let initial_result = adapter.get_entry("cn=Test Modify,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    
    let initial_modify_time = initial_result.attributes.get("modifytimestamp")
    ...
    backend.modify_entry("cn=Test Modify,ou=users,dc=example,dc=com", modifications).await.unwrap();
    
    // Search again and verify modifyTimestamp changed
    let updated_result = adapter.get_entry("cn=Test Modify,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    
    let updated_modify_time = updated_result.attributes.get("modifytimestamp")
        .expect("Should have modifyTimestamp")
        .first()
        .expect("Should have value")
        .clone();
    ...
    assert_ne!(initial_modify_time, updated_modify_time, "modifyTimestamp should be updated");
    
    // Verify user attribute was updated
    assert_eq!(updated_result.attributes.get("cn").unwrap()[0], "Modified Name");
}
...
async fn test_e2e_lmdb_operational_attrs() {
    ...
    for i in 1..=5 {
        let dn = format!("uid=user{},ou=users,dc=example,dc=com", i);
        let result = adapter.get_entry(&dn, &requested_attrs)
            .await
            .unwrap()
            .expect("Entry should exist");
        
        // Should have requested user attribute
        assert_eq!(result.attributes.get("uid").unwrap()[0], format!("user{}", i));
        
        // Should have requested operational attribute
        assert!(result.attributes.contains_key("entrycsn"), "Should have entryCSN");
        
        // Should NOT have non-requested attributes
        assert!(!result.attributes.contains_key("cn"), "Should not have non-requested cn");
        assert!(!result.attributes.contains_key("mail"), "Should not have non-requested mail");
        assert!(!result.attributes.contains_key("createtimestamp"), "Should not have non-requested createTimestamp");
    }
}
...
async fn test_e2e_context_csn_queryable() {
    ...
    assert!(context_csn.is_some(), "contextCSN should be set after adding entry");
    ...
    assert!(csn_value.to_ldap_string().contains('#'), "CSN should be in LDAP format");
}
...
async fn test_e2e_star_excludes_operational_attrs() {
    ...
    let requested_attrs = vec!["*".to_string()];
    let result = adapter.get_entry("cn=Test,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
