...
use opendr::extended_op_fsm::{
    ExtendedOpAccessControl, ExtendedOpBackend, ExtendedOpDelegator, ExtendedOpError,
    ExtendedOpFsmImpl, ExtendedOpMetrics, ExtendedOpParser, ExtendedOperationType, ParsedOperation,
};
use opendr::fsm::{
...
async fn test_extended_op_fsm_access_denied() {
    ...
    let parser = Box::new(MockParser);
    let delegator = Box::new(MockDelegator);
    let access_control = Box::new(MockAccessControl {
        allow_access: false,
    });
    let metrics = Box::new(MockMetrics);
...
#[tokio::test]
async fn test_extended_op_invalid_transition_is_typed_and_state_is_unchanged() {
    let mut fsm = create_test_fsm();

    let result = fsm.handle_event(ExtendedOpEvent::ProcessingComplete).await;

    match result.unwrap_err() {
        ExtendedOpError::InvalidStateTransition { from, event } => {
            assert_eq!(from, ExtendedOpState::Parsing);
            assert!(matches!(event, ExtendedOpEvent::ProcessingComplete));
        }
        other => panic!("Expected typed invalid transition error, got {:?}", other),
    }
...
fn test_parsed_operation_creation() {
    ...
    assert_eq!(
        parsed_op.operation_type,
        ExtendedOperationType::Custom("test_operation".to_string())
    );
...
fn test_extended_op_error() {
    ...
    assert_eq!(
        format!("{}", error),
        "Extended operation error: Test error message"
    );
