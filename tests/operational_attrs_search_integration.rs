...
fn create_test_entry_with_op_attrs(
    dn: &str,
    cn: &str,
    mail: &str,
    replica_id: u16,
    ...
    let op_attrs =
        OperationalAttributes::for_new_entry(csn, Some("cn=admin,dc=example,dc=com".to_string()));
...
async fn test_search_without_operational_attrs_mockbackend() {
    ...
    let result = adapter
        .get_entry("cn=john,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
    ...
    assert_eq!(
        result.attributes.get("mail").unwrap()[0],
        "john@example.com"
    );
    ...
    assert!(
        ...
        "Should not contain entrycsn"
    );
    assert!(
        !result.attributes.contains_key("createtimestamp"),
        "Should not contain createtimestamp"
    );
    assert!(
        !result.attributes.contains_key("modifytimestamp"),
        "Should not contain modifytimestamp"
    );
}
...
async fn test_search_with_plus_all_operational_attrs_mockbackend() {
    ...
    let result = adapter
        .get_entry("cn=jane,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    ...
    assert!(
        ...
        "Should not contain cn when only '+' requested"
    );
    assert!(
        !result.attributes.contains_key("mail"),
        "Should not contain mail when only '+' requested"
    );

    // Should have operational attributes
    assert!(
        result.attributes.contains_key("entrycsn"),
        "Should contain entrycsn"
    );
    assert!(
        result.attributes.contains_key("createtimestamp"),
        "Should contain createtimestamp"
    );
    assert!(
        result.attributes.contains_key("modifytimestamp"),
        "Should contain modifytimestamp"
    );
...
async fn test_search_with_star_and_plus_mockbackend() {
    ...
    let result = adapter
        .get_entry("cn=bob,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
...
async fn test_search_specific_operational_attr_mockbackend() {
    ...
    let result = adapter
        .get_entry("cn=alice,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    ...
    assert!(
        !result.attributes.contains_key("createtimestamp"),
        "Should not contain non-requested createtimestamp"
    );
}
...
async fn test_search_mixed_user_and_operational_attrs_mockbackend() {
    ...
    let result = adapter
        .get_entry("cn=charlie,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    ...
    assert!(
        !result.attributes.contains_key("mail"),
        "Should not contain non-requested mail"
    );
...
async fn test_search_without_operational_attrs_lmdb() {
    ...
    let requested_attrs = vec!["cn".to_string(), "mail".to_string()];
    let result = adapter
        .get_entry("cn=david,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
...
async fn test_search_with_plus_all_operational_attrs_lmdb() {
    ...
    let requested_attrs = vec!["+".to_string()];
    let result = adapter
        .get_entry("cn=emma,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    ...
    assert!(
        entry_csn.contains('#'),
        "entryCSN should contain '#' separators"
    );
}
...
async fn test_search_with_star_and_plus_lmdb() {
    ...
    let requested_attrs = vec!["*".to_string(), "+".to_string()];
    let result = adapter
        .get_entry("cn=frank,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
...
async fn test_search_specific_operational_attr_lmdb() {
    ...
    let result = adapter
        .get_entry("cn=grace,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    ...
    assert!(
        !result.attributes.contains_key("modifytimestamp"),
        "Should not contain non-requested modifytimestamp"
    );
}
...
async fn test_search_case_insensitive_operational_attrs() {
    ...
    let requested_attrs = vec!["ENTRYCSN".to_string(), "CreateTimestamp".to_string()];
    let result = adapter
        .get_entry("cn=henry,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    ...
    assert!(
        result.attributes.contains_key("entrycsn"),
        "Should contain entrycsn (case-insensitive)"
    );
    assert!(
        result.attributes.contains_key("createtimestamp"),
        "Should contain createtimestamp (case-insensitive)"
    );
}
...
async fn test_search_empty_attrs_defaults_to_user_only() {
    ...
    let result = adapter
        .get_entry("cn=iris,ou=users,dc=example,dc=com", &requested_attrs)
        .await
        .unwrap()
        .expect("Entry should exist");
    ...
    assert!(
        result.attributes.contains_key("cn"),
        "Should contain user attributes by default"
    );
    assert!(
        result.attributes.contains_key("mail"),
        "Should contain user attributes by default"
    ...
    assert!(
        !result.attributes.contains_key("entrycsn"),
        "Should not contain operational attrs by default"
    );
}
