...
async fn test_changelog_provider_with_csn() {
    ...
    let entries = provider
        .get_changelog_since(Some(&cookie1), 10)
        .await
        .unwrap();
...
async fn test_csn_incremental_sync() {
    ...
    let _csn1 = changelog.record_change(
        ChangeType::Add,
        "cn=user1,dc=example,dc=org".to_string(),
        b"user1".to_vec(),
    );
    ...
    let initial_entries = provider
        .get_changelog_since(Some(initial_cookie), 100)
        .await
        .unwrap();
    ...
    let incremental_entries = provider
        .get_changelog_since(Some(&last_cookie), 100)
        .await
        .unwrap();
