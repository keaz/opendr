...
async fn test_operational_attributes_creation() {
    ...
    assert_eq!(
        op_attrs.creators_name,
        Some("cn=admin,dc=example,dc=org".to_string())
    );
    assert_eq!(
        op_attrs.modifiers_name,
        Some("cn=admin,dc=example,dc=org".to_string())
    );
}
...
async fn test_operational_attributes_modification() {
    let csn1 = Csn::new(1);
    let mut op_attrs =
        OperationalAttributes::for_new_entry(csn1, Some("cn=admin,dc=example,dc=org".to_string()));
    ...
    assert_eq!(
        op_attrs.modifiers_name,
        Some("cn=user,dc=example,dc=org".to_string())
    );
...
async fn test_directory_entry_with_operational_attrs() {
    ...
    let entry =
        DirectoryEntry::with_operational_attrs("cn=test,dc=example,dc=org", attributes, op_attrs);
