...
async fn test_cookie_file_creation() {
    ...
    // Verify file exists
    let cookie_path: PathBuf = [
        temp_dir.path(),
        std::path::Path::new("replication_cookie.txt"),
    ]
    .iter()
    .collect();
...
async fn test_cookie_whitespace_handling() {
    ...
    assert_eq!(
        loaded,
        Some("csn-20251007123456789012#001#000001#000000".to_string())
    );
}
...
#[tokio::test]
async fn test_save_cookie_fails_when_storage_path_is_a_file() {
    let temp_dir = TempDir::new().unwrap();
    let file_path = temp_dir.path().join("state-file");
    std::fs::write(&file_path, "not-a-directory").unwrap();

    let manager = StateManagerImpl::new(file_path.to_string_lossy().into_owned());
    let result = manager
        .save_cookie("csn-20251007123456789012#001#000001#000000")
        .await;

    assert!(result.is_err());
    assert!(
        !file_path.join("replication_cookie.txt").exists(),
        "cookie file should not exist when the storage path is invalid"
    );
}
