...
async fn test_csn_replication_full_workflow() {
    ...
    assert!(csn2 > csn1, "CSN ordering: csn2 should be greater than csn1");
    
    // Phase 2: Consumer performs initial sync (full refresh)
    let provider_impl = ChangelogProviderImpl::new(provider_changelog.clone(), provider_backend.clone());
    
    // Get all entries for full refresh (cookie is empty)
    let refresh_entries = provider_impl.get_all_entries("dc=example,dc=org", None).await.unwrap();
    assert_eq!(refresh_entries.len(), 2);
    
    // Apply to consumer
    for entry in refresh_entries {
        consumer_backend.add_entry(DirectoryEntry::new(entry.dn, entry.attributes), vec![]).await.unwrap();
    }
    ...
    let sync_cookie = provider_impl.generate_cookie(&context_csn_value).await.unwrap();
    ...
    provider_wrapper.modify_entry("cn=john,dc=example,dc=org", modifications).await.unwrap();
    
    // Delete an entry
    provider_wrapper.delete_entry("cn=jane,dc=example,dc=org").await.unwrap();
    
    // Phase 4: Consumer performs incremental sync
    let incremental_changes = provider_impl.get_changelog_since(Some(&sync_cookie), 100).await.unwrap();
    ...
    assert!(new_context_csn > context_csn_value, 
...
async fn test_csn_multi_replica_replication() {
    ...
    assert!(replica2_changes[0].csn > replica1_changes[0].csn,
...
async fn test_csn_replication_resume_after_disconnect() {
    ...
    let resumed_changes = provider.get_changelog_since(Some(&saved_cookie), 100).await.unwrap();
...
async fn test_csn_replication_with_operations() {
    ...
    wrapper.modify_entry("cn=test,dc=example,dc=org", modifications).await.unwrap();
    
    // 3. Rename
    wrapper.rename_entry("cn=test,dc=example,dc=org", "cn=renamed", true, None).await.unwrap();
    
    // 4. Delete
    wrapper.delete_entry("cn=renamed,dc=example,dc=org").await.unwrap();
