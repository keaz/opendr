...
async fn test_lmdb_basic_crud() {
    ...
    let entry = DirectoryEntry::new("cn=John Doe,ou=people,dc=example,dc=org", attributes.clone());
    backend.add_entry(entry, b"secret".to_vec()).await.unwrap();

    // Read
    let retrieved = backend.get_entry("cn=John Doe,ou=people,dc=example,dc=org").await.unwrap();
    assert!(retrieved.is_some());
    let retrieved = retrieved.unwrap();
    assert_eq!(retrieved.dn, "cn=John Doe,ou=people,dc=example,dc=org");
    assert_eq!(retrieved.attributes.get("cn").unwrap(), &vec!["John Doe".to_string()]);
    ...
    backend.modify_entry("cn=John Doe,ou=people,dc=example,dc=org", modifications).await.unwrap();

    let updated = backend.get_entry("cn=John Doe,ou=people,dc=example,dc=org").await.unwrap().unwrap();
    assert!(updated.attributes.contains_key("telephonenumber")); // normalized

    // Delete
    backend.delete_entry("cn=John Doe,ou=people,dc=example,dc=org").await.unwrap();
    let deleted = backend.get_entry("cn=John Doe,ou=people,dc=example,dc=org").await.unwrap();
...
async fn test_lmdb_persistence() {
    ...
    // Reopen backend and verify data persists
    {
        ...
        let auth = backend.authenticate("cn=persistent,dc=example,dc=org", b"password").await.unwrap();
...
async fn test_lmdb_concurrent_reads() {
    ...
    for i in 0..100 {
        ...
        backend.add_entry(entry, format!("pass{}", i).as_bytes().to_vec()).await.unwrap();
    }
...
async fn test_lmdb_search_operations() {
    ...
    let subtree_results = backend.search_entries("dc=example,dc=org", SearchScope(2)).await.unwrap();
    assert_eq!(subtree_results.len(), 10, "Subtree should find exactly 10 entries");

    // Test with more specific base
    let subtree_results2 = backend.search_entries("ou=people,dc=example,dc=org", SearchScope(2)).await.unwrap();
    assert_eq!(subtree_results2.len(), 10, "Subtree under ou=people should find 10 entries");
}
...
async fn test_lmdb_modify_operations() {
    ...
    // Test Add operation
    backend.modify_entry("cn=Test,dc=example,dc=org", vec![
        Modification {
            operation: ModifyOperation::Add,
    ...
    let result = backend.get_entry("cn=Test,dc=example,dc=org").await.unwrap().unwrap();
    ...
    let result = backend.get_entry("cn=Test,dc=example,dc=org").await.unwrap().unwrap();
    assert_eq!(result.attributes.get("mail").unwrap(), &vec!["replaced@example.org".to_string()]);
...
async fn test_lmdb_rename_operations() {
    ...
    // Rename entry
    backend.rename_entry("cn=oldname,dc=example,dc=org", "cn=newname", true, None).await.unwrap();

    // Verify old entry doesn't exist
    let old_entry = backend.get_entry("cn=oldname,dc=example,dc=org").await.unwrap();
    assert!(old_entry.is_none());

    // Verify new entry exists
    let new_entry = backend.get_entry("cn=newname,dc=example,dc=org").await.unwrap();
    assert!(new_entry.is_some());

    // Verify authentication works with new DN
    let auth = backend.authenticate("cn=newname,dc=example,dc=org", b"password").await.unwrap();
    assert!(auth);
}
...
async fn test_lmdb_compare_operations() {
    ...
    let result = backend.compare_attribute("cn=Test,dc=example,dc=org", "mail", "test@example.org").await.unwrap();
    assert!(result, "Compare should match existing value");

    // Test compare doesn't match
    let result = backend.compare_attribute("cn=Test,dc=example,dc=org", "mail", "wrong@example.org").await.unwrap();
    assert!(!result, "Compare should not match wrong value");

    // Test compare non-existent attribute
    let result = backend.compare_attribute("cn=Test,dc=example,dc=org", "telephonenumber", "123").await.unwrap();
    assert!(!result, "Compare should return false for non-existent attribute");
}
...
async fn test_lmdb_duplicate_prevention() {
    ...
    backend.add_entry(entry1, b"password".to_vec()).await.unwrap();
...
async fn test_lmdb_error_handling() {
    ...
    assert!(result.is_err(), "Should error on deleting non-existent entry");

    // Test modify non-existent entry
    let result = backend.modify_entry("cn=nonexistent,dc=example,dc=org", vec![]).await;
    assert!(result.is_err(), "Should error on modifying non-existent entry");

    // Test compare on non-existent entry
    let result = backend.compare_attribute("cn=nonexistent,dc=example,dc=org", "cn", "test").await;
    assert!(result.is_err(), "Should error on comparing non-existent entry");
}
